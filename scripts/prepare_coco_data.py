...
def create_coco_dict(images, annotations, categories, license):
    coco_dict = {}
    coco_dict["images"] = images.to_json(orient="records")
    coco_dict["annotations"] = annotations.to_json(orient="records")
...
def main(cfg_file_path):
    ...
    images_w_annotations_df = pd.DataFrame()
    ...
    for dataset_key, coco_file in COCO_FILES_DICT.items():
        ...
        images_w_annotations_df = pd.concat((images_w_annotations_df, pd.DataFrame(coco_data)), ignore_index=True)

    if DEBUG:
        logger.info("Debug mode activated. Only first 100 images are processed.")
        images_w_annotations_df = images_w_annotations_df.head(100)

    # Iterate through images and clip them into tiles
    overlap_x = 112
    overlap_y = 6   # ok values: 6 px (5 tiles) or 282 px (11 tiles)
    padding = 735
    image_id = 0
    annotation_id = 0
    ...
    for image in tqdm(images_w_annotations_df.itertuples(), desc="Clipping images and annotations into tiles", total=len(images_w_annotations_df)):
        ...
        for i in range(padding, h - padding, TILE_SIZE - overlap_y):
            for j in range(0, w, TILE_SIZE - overlap_x):
                tile = img[i:i+TILE_SIZE, j:j+TILE_SIZE]
                new_filename = f"{os.path.basename(image.file_name).rstrip('.jpg')}_{i}_{j}.jpg"

                tiles.append({"height": TILE_SIZE, "width": TILE_SIZE, "image_id": image_id, "file_name": new_filename})
                cv2.imwrite(os.path.join(OUTPUT_DIR_IMAGES, new_filename), tile)
                image_id += 1
        ...
        for ann in image.annotations:
            for tile in tiles:
                ...
                if ann["bbox"][0] + ann["bbox"][2] <= tile_min_x or ann["bbox"][0] >= tile_max_x or ann["bbox"][1] + ann["bbox"][3] <= tile_min_y or ann["bbox"][1] >= tile_max_y:
                    continue
                # else, adapt coordinates and clip if necessary
                x1 = max(ann["bbox"][0] - tile_min_x, 0)
                y1 = max(ann["bbox"][1] - tile_min_y, 0)
                x2 = ann["bbox"][2] if x1 + ann["bbox"][2] <= TILE_SIZE else TILE_SIZE - x1
                y2 = ann["bbox"][3] if y1 + ann["bbox"][3] <= TILE_SIZE else TILE_SIZE - y1
        ...
        if len(annotations) == 0:
            ...
            remove_discarded_tiles(all_tiles_df, all_tiles_df.sample(n=2, random_state=SEED).file_name.unique(), OUTPUT_DIR_IMAGES)
            ...
            tiles_with_ann_df = all_tiles_df[all_tiles_df["image_id"].isin(tile_ids_w_annotations)]
            tiles_without_ann_df = all_tiles_df[~all_tiles_df["image_id"].isin(tile_ids_w_annotations)]
            ...
            remove_discarded_tiles(tiles_without_ann_df, tiles_without_ann_df.sample(n=nbr_tiles_without_ann, random_state=SEED).file_name.unique(), OUTPUT_DIR_IMAGES)
    ...
    val_tiles = tiles_df[~tiles_df["image_id"].isin(trn_tiles["image_id"])].sample(frac=0.5, random_state=SEED)
    tst_tiles = tiles_df[~tiles_df["image_id"].isin(trn_tiles["image_id"].to_list() + val_tiles["image_id"].to_list())]
    ...
    for dataset in dataset_tiles_dict.keys():
        # Split annotations
        dataset_annotations = clipped_annotations_df[clipped_annotations_df["image_id"].isin(dataset_tiles_dict[dataset]["image_id"])]
